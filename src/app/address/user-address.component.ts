...
export class UserAddressComponent implements AfterViewInit, OnDestroy {
  ...
  protected get qparams() {
    return { commandId: 'list', addressId: this._parentId };
  }
  ...
  constructor(
    ...
    let textlib = [users, examinators, textdata, panelbuttons];
  ...
  doDeleteItem(action: string) {
    if (action == 'YES') {
      let crudcmd = Object.assign({}, { 'oid': this.oid, 'cid': 'del', 'qparams': { 'addressId': this._parentId } });
