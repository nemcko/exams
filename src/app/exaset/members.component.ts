...
@Component({
  selector: 'exasetmbrs',
  template: `
<div class="text-primary px-1">{{labels.btnMembers}}</div>
<brw [uuid]="uuid" [oid]="oid"  [service]="service" [qparams]="qparams">
  <brwcol [name]="'usr'" [header]="labels.usr" [width]="10"></brwcol>
  <brwcol [name]="'firstname'" [header]="labels.firstname" [width]="50"></brwcol>
...
<pgalert [dlgid]="'deleteItemalertdlg'" [dlgRef]="deleteItemalertdlg" [title]="labels.delMemberTitle" [type]="'YES-NO'"
  (doClick)="doDeleteItem($event)">
  <p>{{labels.delMemberQuestion}}</p>
</pgalert>  
`
})
export class MembersComponent implements AfterViewInit, OnDestroy {
  ...
  protected get qparams() {
    return { commandId: 'members', userId: this._parentId, 'examtype': this.examtype };
  }
  ...
  doDeleteItem(action: string) {
    if (action == 'YES') {
      let crudcmd = Object.assign({}, { 'oid': 'exagrps', 'cid': 'delmember', 'qparams': { 'clientId': this._parentId , 'examtype': this.examtype} });
