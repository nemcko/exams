...
@Component({
  selector: 'exasetusrs',
  template: `
  <div class="text-primary px-1">{{labels.btnUsers}}</div>
...
<pgalert [dlgid]="'deleteItemalertdlg'" [dlgRef]="deleteItemalertdlg" [title]="labels.delUserTitle" [type]="'YES-NO'"
  (doClick)="doDeleteItem($event)">
  <p>{{labels.delUserQuestion}}</p>
</pgalert>  
`
})
export class UsersComponent implements AfterViewInit, OnDestroy {
  ...
  doDeleteItem(action: string) {
    if (action == 'YES') {
      let crudcmd = Object.assign({}, { 'oid': 'exagrps', 'cid': 'deluser', 'qparams': { 'clientId': this._parentId , 'examtype': this.examtype} });
